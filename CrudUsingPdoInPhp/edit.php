<?php
    ...
    try {
        $db_query = "SELECT id, name, email, website, comment, image_path, docu_path FROM employees WHERE id = '$id' LIMIT 0,1";
...
<div class="form-data">
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?id={$id}");?>" enctype="multipart/form-data">
        ...
        <div class="bottom-margin">
        <a href="<?php echo $data['docu_path']; ?>"> <?php 
                    $file = $data['docu_path'];
                    echo str_replace("uploads/","",$file);
                     ?>
                      </a> 
        </div>
        <div class="bottom-margin">
